...
def _load_html_files():
    ...
    # Use absolute path relative to this script — works regardless of Railway working dir
    base_dir = os.path.dirname(os.path.abspath(__file__))

    for fname, varname in [('login.html', '_login_html'), ('index.html', '_dashboard_html')]:
        # Try script directory first, then current working directory as fallback
        for path in [os.path.join(base_dir, fname), os.path.join(os.getcwd(), fname), fname]:
            try:
                with open(path, 'r', encoding='utf-8') as f:
                    content = f.read()
                if varname == '_login_html':
                    _login_html = content
                else:
            ...
            log(f'  ⚠️  Could not find {fname} — searched {base_dir} and {os.getcwd()}')
