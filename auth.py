...
============================================================
Uses Mailjet API for OTP emails (free 200/day, no domain or phone needed).
Sign up at mailjet.com with just an email address.

Render Environment Variables needed:
  MAILJET_API_KEY     — from mailjet.com → Account → API Keys
  MAILJET_SECRET_KEY  — from mailjet.com → Account → API Keys
  SMTP_EMAIL          — verified sender email (indiastockhit@gmail.com)
  JWT_SECRET          — python3 -c "import secrets; print(secrets.token_hex(32))"
  APP_NAME            — optional (default: India Stock AI)
  GEMINI_API_KEY      — injected into dashboard at login time
"""
...
JWT_SECRET         = os.environ.get('JWT_SECRET',         'change-me-set-in-render')
...
# ── Email via Mailjet API ─────────────────────────────────────────────────────
def _send_otp_email(to_email, otp):
    """
    Send OTP via Mailjet API.
    Free 200 emails/day. No domain or phone number needed.
    Uses HTTP Basic Auth with API key + Secret key.
    """
    if not MAILJET_API_KEY or not MAILJET_SECRET_KEY:
        print('[AUTH] MAILJET_API_KEY or MAILJET_SECRET_KEY not set in Render Variables')
        return False, 'Email service not configured. Add MAILJET_API_KEY and MAILJET_SECRET_KEY in Render Variables.'
    if not SMTP_EMAIL:
        print('[AUTH] SMTP_EMAIL not set in Render Variables')
        return False, 'Sender email not configured. Add SMTP_EMAIL in Render Variables.'
    ...
    payload = _json.dumps({
        "Messages": [{
            ...
            "HTMLPart": html_body,
            "TextPart": f"{APP_NAME} OTP: {otp}\nValid 10 minutes. Do not share.",
        }]
    }).encode("utf-8")

    # Mailjet uses HTTP Basic Auth: API Key as username, Secret Key as password
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=15) as resp:
            ...
            print(f"[AUTH] OTP sent to {to_email} via Mailjet (status: {status})")
            return True, ''

    except urllib.error.HTTPError as e:
        body = e.read().decode(errors='ignore')
        print(f"[AUTH] Mailjet HTTP {e.code}: {body[:200]}")
        if e.code == 401:
            return False, "Invalid Mailjet credentials — check MAILJET_API_KEY and MAILJET_SECRET_KEY in Render Variables."
        if e.code == 400:
            return False, f"Mailjet rejected request — make sure '{SMTP_EMAIL}' is verified in your Mailjet account."
        return False, f"Email API error (HTTP {e.code}). Please try again."
    except Exception as e:
        print(f"[AUTH] Mailjet error: {e}")
